...
class yaj():
	def __init__(self,meshpath,flowmode,dnspath,import_flag,m,eta,n_eta):
		#physical parameters
		self.Re=1000
		self.U =1
		self.D =2
		...
		self.n_eta=n_eta #number of swirl iterations
	...
	def BoundaryGeometry(self):
		...
		def outlet(	 x, on_boundary):    #sortie
			return x[0] > 40-self.eps   and on_boundary 
	...
	def BoundaryConditions(self,eta):
		...
		if self.label=='incompressible':
			...
			uth_tanh = Expression(str(eta)+'*tanh(5*(1-x[1])*x[1])', degree=2)
			bcs_wall	  	= DirichletBC(self.Space.sub(0),	   (0,0,0),   wall)  	# No flow at wall
			bcs_symmetry_r	= DirichletBC(self.Space.sub(0).sub(1), 0, 	      symmetry) # No radial flow through central axis
			bcs_symmetry_th = DirichletBC(self.Space.sub(0).sub(2), 0, 	      symmetry) # No azimuthal flow either
	...
	def Newton(self):
		etas= np.cos(np.pi*np.linspace(self.n_eta,0,self.n_eta)/2/self.n_eta)*self.eta #Chebychev spacing
		for eta_current in etas: 	#increase swirl
