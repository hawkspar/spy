...
if p0:
	if isfile(dir+"avg.pkl"):
		with open(dir+'avg.pkl', 'rb') as fp: avgs=load(fp)
	else: avgs={}
	if isfile(dir+"var.pkl"):
		with open(dir+'var.pkl', 'rb') as fp: vars=load(fp)
	else: vars={}
	for S in Ss:
...
for dat in dats:
	...
	int=dfx.fem.assemble_scalar(dfx.fem.form(chi*ufl.dx)).real
	int=comm.gather(int)
	if p0:
		avg = sum(int)/A
		avgs[str(S)][str(m)][str(St)] = avg
	avg = comm.scatter([avg for _ in range(comm.size)])
	int=dfx.fem.assemble_scalar(dfx.fem.form((chi-avg)**2*ufl.dx)).real
	int=comm.gather(int)
	if p0: vars[str(S)][str(m)][str(St)] = sum(int)/A

# Writing everything into a double contour plot
if p0:
	for name,res in [('avg',avgs),('var',vars)]:
