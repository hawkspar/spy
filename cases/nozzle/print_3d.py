...
# Parameters range
Ss=np.linspace(0,1.2,4)
ms=range(-2,3)
Sts=[.05, .48, 1.13, 1.57, 2.]
# Actual plotting
...
YZf_cr = np.hstack((np.linspace(0,1,25,endpoint=False), np.linspace(1,1.1,25)))
...
for S in Ss:
	for m in ms:
		for St in Sts:
			file_name=dir+f"Re={Re:d}_S={S}_m={m:d}_St={St:.2f}".replace('.',',')+".html"
