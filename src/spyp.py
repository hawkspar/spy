...
class SPYP(SPY):
	def __init__(self, params:dict, datapath:str, Ref, nutf,
				 boundaryConditions, S:float, m:int) -> None:
		super().__init__(params, datapath, Ref, nutf)
		self.S=S; self.m=m
		self.save_string='_S='+f"{S:00.3f}"+'_m='+str(m)
		self.boundaryConditions=lambda m: boundaryConditions(self,m)
	...
	def assembleMatrices(self) -> None:
		...
		self.boundaryConditions(self.m)
	...
	def resolvent(self,k:int,St_list):
		...
		for St in St_list:
			...
			EPS.setTolerances(self.params['atol'],100) # Set absolute tolerance and number of iterations
			EPS.setFromOptions()
